...
class RecSys(InMemoryDataset):
    ...
    def raw_dir(self):
        if self.FL:
            return osp.join(self.root, 'FL'+self.name, 'raw')
        else:
    ...
    def processed_dir(self):
        if self.FL:
            return osp.join(self.root, 'FL'+self.name, 'processed')
        else:
            return osp.join(self.root, self.name, 'processed')

    def download(self):
        # Download to `self.raw_dir`.
        url = 'https://raw.githubusercontent.com/FedML-AI/FedGraphNN/main/data/recommender_system/'
