...
def get_gnn(args, dataset):
    if args.task == 'node_level':
        if args.model_type == 'gcn':
            # assume `data` is a dict where key is the client index, and value is a PyG object
            model = GCN_Net(dataset.num_features,
                            dataset.num_classes,
                            hidden=args.hidden,
                            max_depth=args.nlayers,
                            dropout=args.dropout)
        elif args.model_type == 'sage':
            model = SAGE_Net(dataset.num_features,
                             dataset.num_classes,
                             hidden=args.hidden,
                             max_depth=args.nlayers,
                             dropout=args.dropout)
        elif args.model_type == 'gat':
            model = GAT_Net(dataset.num_features,
                            dataset.num_classes,
                            hidden=args.hidden,
                            max_depth=args.nlayers,
                            dropout=args.dropout)
        elif args.model_type == 'gin':
            model = GIN_Net(dataset.num_features,
                            dataset.num_classes,
                            hidden=args.hidden,
                            max_depth=args.nlayers,
                            dropout=args.dropout)
        elif args.model_type == 'gpr':
            model = GPR_Net(dataset.num_features,
                            dataset.num_classes,
                            hidden=args.hidden,
                            K=args.nlayers,
                            dropout=args.dropout)
        ...
        model = GNN_Net_Link(dataset.num_features,
                             dataset.num_classes,
                             hidden=args.hidden,
                             max_depth=args.nlayers,
                             dropout=args.dropout,
        ...
        if args.model_type == 'mpnn':
            model = MPNNs2s(in_channels=dataset.num_features,
                            out_channels=dataset.num_classes,
                            num_nn=dataset.num_edge_features,
                            hidden=args.hidden)
        else:
            model = GNN_Net_Graph(dataset.num_features,
                                  dataset.num_classes,
                                  hidden=args.hidden,
                                  max_depth=args.nlayers,
                                  dropout=args.dropout,
